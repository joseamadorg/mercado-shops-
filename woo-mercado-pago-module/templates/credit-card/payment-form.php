   ...
   (function() {
                  ...
                  if (request.status == 200) {
                     var response = JSON.parse(request.responseText);
                        ...
                        document.querySelector(MPv1.selectors.mpCouponApplyed).style.display =
                           ...
                           MPv1.text.discount_info2 + " " + response.response.name + ".<br>" +
									MPv1.text.discount_info3 + " <strong>" +
                           MPv1.currencyIdToCurrency(response.response.currency_id) + " " +
                           Math.round(MPv1.getAmountWithoutDiscount()*100)/100 + "</strong><br>" +
									MPv1.text.discount_info4 + " <strong>" +
                           MPv1.currencyIdToCurrency(response.response.currency_id) + " " +
                           Math.round(MPv1.getAmount()*100)/100 + "*</strong><br>" + "<i>" +
                           MPv1.text.discount_info5 + "</i><br>" +
            ...
            if (MPv1.site_id != "MLM") {
               ...
               if (MPv1.customer_and_card.status) {
                  document.querySelector(MPv1.selectors.paymentMethodSelector).style.background =
                     "url(" + response[0].secure_thumbnail + ") 95% 50% no-repeat #fff";
               } else {
         ...
         if (MPv1.site_id == "MLM") {
            params_installments = {
               "payment_method_id": document.querySelector(
                  MPv1.selectors.paymentMethodSelector).value,
               "amount": amount,
               "issuer_id": issuerId
            }
         }
      ...
      MPv1.setInstallmentInfo = function(status, response) {
         ...
         if (response.length > 0) {
            ...
            for (var i = 0; i < payerCosts.length; i++) {
               ...
               (payerCosts[i].recommended_message || payerCosts[i].installments) + "</option>";
            }
      ...
      MPv1.setForm = function() {
         if (MPv1.customer_and_card.status) {
            ...
            document.querySelector(MPv1.selectors.mpSecurityCodeCustomerAndCard).style.display =
               "none";
      ...
      MPv1.addListenerEvent(
         document.querySelector(MPv1.selectors.cardNumber), "keyup", MPv1.guessingPaymentMethod);
      MPv1.addListenerEvent(
         document.querySelector(MPv1.selectors.cardNumber), "keyup", MPv1.clearOptions);
      MPv1.addListenerEvent(
         document.querySelector(MPv1.selectors.cardNumber), "change", MPv1.guessingPaymentMethod);
